...
# ---------------- Import Wave 1 datasets ----------------
# Import wave 1 main and weight dataset
w1_main <- read_sav(
...
w1_selected <- w1_main %>%
  select(
    ...
    # Wave 1 variables
    H1DA7,H1DA4,H1ED1,H1GH53,H1ED2,H1ED9,H1ED7,H1DS7,H1DA3,H1DA11,H1DA10,
    ...
    matches("^H1HR9[A-T]$"),H1FP24A1,H1FP24A2,H1TO29,H1TO9,H1TO33
  )
...
w2_selected <- w2_main %>%
  select(
    ...
    h2da7,h2da4,h2ed1,h2gh46,h2ds5,h2ed2,h2ed5,h2ed3,h2da3,h2da11,h2da10,h2da8,
    h2da9,h2re3,h2re7,h2da5,matches("^h2gh33[a-g]$"),matches("^h2gh32[a-g]$"),
    h2da1,h2gh44,h2ee4,h2ee6,h2to60,h2to52,h2to64,h2to61,h2to58,h2to50,
    ...
    h2re1,h2rr2a,h2gi5,matches("^h2hr2[a-q]$"),h2fp28a1,h2fp28a2,h2fp28a3,
    h2ds14,h2to41,h2to10,h2to48
  ) %>%
  rename(AID = aid)
...
w3_selected <- w3_main %>%
  select(
    ...
    h3da15,h3da40,h3ed33,h3hr23,h3da3,h3da5,h3da7,h3da4,h3da6,h3cc3,h3cc9a,
    ...
    h3ds11,h3ds16,h3ds7,h3ec4,h3ec2,h3ec5,h3ec3,h3ec7,h3ec6,h3ec40,h3ec62,
    h3od1m,h3od1y,bio_sex3,matches("^h3od4[a-d]$"),
    h3cj160,h3cj152,prison3,h3re1,h3hr6,h3ds13,
    h3to104,h3to103
  ) %>%
  rename(AID = aid)

w3_selected <- left_join(
  w3_selected,
  w3_weight,
  by = "AID") %>%
...
# four IPV-related DV + one relationship-related variable
w3_IPV <- w3_partnerdetail %>%
  select(
    AID,
    H3RD116,H3RD113,H3RD111,H3RD109,H3RD3
  )

# two partner relationship variables
...
# one having child variable
w3_childliving <- w3_birth %>%
  select(
    AID,
    H3LB11
  )

# ---------------- Import Wave 4 datasets ----------------
w4_main <- read_sav(
...
w4_selected <- w4_main %>%
  select(
    ...
    H4DA23,H4DA22,H4DA1,H4DA26,H4RE7,H4RE8,H4DA6,H4DA5,H4DA4,H4DA7,H4DA13,
    H4LM19,H4LM13,H4TO67,H4TO65A,H4TO65B,H4TO65C,H4TO65D,H4TO65E,H4DS19,
    ...
    H4WP9,H4CJ24M,H4CJ17,PRISON4,H4RE1,H4TR4,H4HR4
  )

w4_selected <- left_join(
  w4_selected,
  w4_weight,
  by = "AID") %>%
...
# one having child variable
w4_childliving <- w4_birth %>%
  select(
    AID,
    H4LB10
  )

# ---------------- Import Wave 5 datasets ----------------
...
w5_selected <- w5_main %>%
  select(
    ...
    H5SS1,H5SS2,H5ID23,H5SS7,H5RE2,H5ID26,H5ID27,H5ID28,H5ID15,H5LM7,H5TO27A,
    ...
    H5TR5,H5TR4,H5HR3,matches("^H5PG26[1-6]$")
  )
...
w6_selected <- w6_main %>%
  select(
    ...
    H6CC1,H6CC2,H6SS1,H6RE2,H6DA10,H6DA11,H6SP1,matches("^H6LM4[A-E]$"),
    ...
    H6TR6,H6TR3,H6HR1,H6HR3,matches("^H6PG22[A-F]$"),
    matches("^H6PG26[A-F]$")
  )
