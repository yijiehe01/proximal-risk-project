...
w1_selected <- w1_main %>%
  select(
    ...
    matches("^H1HR2[A-T]$"),H1FP24A1,H1FP24A2,H1TO29,H1TO9,H1TO33,H1TO32,H1TO44
  )
...
w3_selected <- w3_main %>%
  select(
    ...
    h3hr6,h3hr5,h3hr2,h3ds13,h3to104,h3to103,h3to108,h3to109,h3to110,h3to112,h3to115
  ) %>%
  rename(AID = aid)
...
w4_selected <- w4_main %>%
  select(
    ...
    H4WP9,H4CJ24M,H4CJ17,PRISON4,H4RE1,H4TR4,H4HR4,H4HR3,H4HR1,H4TR11
  )
...
w6_selected <- w6_main %>%
  select(
    ...
    IYEAR6,H6CC1,H6CC2,H6SS1,H6RE2,H6DA10,H6DA11,H6DA12,H6SP1,matches("^H6LM4[A-E]$"),
    ...
    matches("^H6PG26[A-F]$"),matches("^H6HR4[A-H]$")
  )
